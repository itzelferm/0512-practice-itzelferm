...
from square import square, describe_square
from multiplication import multiply, describe_multiply
from subtraction import subtract, describe_subtract
from decrement import decrement, describe_decrement
from division import division, describe_division
...
CALC_MODULES = {
    ...
    'square': {
        'operator': square,
        'describer': describe_square
    },
    'multiplication': {
        'operator': multiply,
        'describer': describe_multiply
    },
    'subtract':{
        'operator': subtract,
    ...
    'decrement':{
        'operator': decrement,
        'describer': describe_decrement
    },
    'division':{
        'operator': division,
        'describer': describe_division
    }
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        ...
        #TODO Making a Friendlier Error Message
        try: 
            answer = calc(operator, operands)
            print(f'The answer is {answer}.')
        except KeyError:
            print(f"Sorry, we don't support the {operator}  operator yet :(. We do support these operators ↩️") 
            print_available_operators()